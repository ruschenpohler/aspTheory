...
label var digitspan_total "Digitspan total (0-8)"

egen digitspan_total_md = median(digitspan_total)
gen digitspan_total_abvmd = 0
replace digitspan_total_abvmd = 1 if digitspan_total>=digitspan_total_md
...
label var time_comp "Time preferences (0=impatient to 10=patient)"
...
label var risk_comp "Risk attitudes (0=risk-averse to 10=risk-seeking)"
...
label var formal_tax "Registration for taxes (yes/no)"
...
label var age_firm "Age of establishment (years)"
...
label var custom_total "Total daily customers"
...
_crcslbl	assets_otherfirm f4_3_9

* Not clear if erroneous: Everybody has at least 1 other business.
gen assets_otherfirm_1 = 0
replace assets_otherfirm_1 = 1 if assets_otherfirm>1
...
label var sales_nday "Total daily sales (USD PPP)"
...
label var prof_lastmth "Self-reported Profits Last Month"
...
label var prof_nday "Total daily profits (USD PPP)
...
label var prof_est "Estimated Profits Last Month"
...
label var prods_new_1 "At least 1 new product for sale in previous 12 months (yes/no)"
...
label var labour_total "Total employees"
...
egen MW_score_total = rowmean(MWM2_visitcompetprod MWM3_askcustomprod MWM4_askcustomquit MWM5_asksupplprod MWM6_attrcustomdisc MWM7_advert ///
MWB1_negosupplprice MWB2_compsupplprod MWB3_notOOS ///
MWR1_recwritten MWR2_recpurchsale MWR3_recliquid MWR4_recsalesprods MWR5_costprods MWR6_profprods MWR7_recexpensemth MWR8_recloan ///
MWF1_finperform MWF2_settargetyr MWF3_comptargetmth MWF4_expensenextyr MWF5_proflossyr MWF6_cashflowyr MWF7_balanceyr MWF8_incexpenseyr)
label var MW_score_total "Business-practices Composite"

* Marketing sub-score
egen MW_M_score_total = rowmean(MWM2_visitcompetprod MWM3_askcustomprod ///
MWM4_askcustomquit MWM5_asksupplprod MWM6_attrcustomdisc MWM7_advert)
label var MW_M_score_total "Marketing Subscore (% score)"

* Stock sub-score
egen MW_B_score_total = rowmean(MWB1_negosupplprice MWB2_compsupplprod MWB3_notOOS)
label var MW_B_score_total "Stock-up Subscore (% score)"
...
label var MW_R_score_total "Record-keeping Subscore (% score)"
...
label var MW_F_score_total "Financial-planning Subscore (% score)"
...
label var newbranch_nextyr "Plans to open business in next 12mth"
...
label var bisplan_nextyr "Plans to develop business plan for next 12 mth"
...
label var startrec_nextyr "Plans to start/improve book-keeping in next 12mth"
...
label var loan_nextyr "Plans to obtain loan in following year (yes/no)"
...
label var vat_nextyr "Plans to register for VAT in next 12mth"
...
label var loan_amt "Total loans outstanding (USD PPP)"
//replace loan_amt =. if loan_amt>20000
gen loan_amt_ihs =  ln(loan_amt + sqrt((loan_amt*loan_amt) + 1))
label var loan_amt_ihs "IHS of total loans outstanding (USD PPP)"
...
label var save_daily "Daily saving (yes/no)"

gen save_daily_fix_amt = f4_8_10/xchange if f4_8_10 < 999999996
gen save_daily_prop_amt = f4_8_11/100*sales_nday
gen save_daily_amt = save_daily_fix_amt
replace save_daily_amt = save_daily_prop_amt if save_daily_fix_amt==.
replace save_daily_amt = 0 if save_daily_fix_amt==. & save_daily_prop_amt==.
label var save_daily_amt "Daily savings (USD PPP)"


***** INVESTMENT

gen reinvest_daily = 0	
replace reinvest_daily = 1 if inlist(f4_8_6, 1,2)
replace reinvest_daily =. if f4_8_6==7==8

gen reinvest_daily_fix_amt = f4_8_7/xchange
gen reinvest_daily_prop_amt = f4_8_8/100*sales_nday 
...
replace reinvest_daily_amt = 0 if reinvest_daily_fix_amt==. & reinvest_daily_prop_amt==.
replace reinvest_daily_amt =. if reinvest_daily_amt>9000
label var reinvest_daily_amt "Daily investment in business (USD PPP)"
...
label var asp_occup_kids_high "Avg occup aspirations for children above md (yes=1)"
label var asp_occup_kids_govt "Gov't job aspired for either son or daughter (yes=1)"
...
replace asp_educ_son = 25 if asp_educ_son>25
...
label var asp_educ_son_high			"Aspired education for son above md (yes=1)"
label var asp_educ_son_ma			"Aspired education for son at least MA (yes=1)"
label var asp_educ_daughter			"Aspired education for daughter (years)"
label var asp_educ_daughter_high	"Aspired education for son above md (yes=1)"
label var asp_educ_daughter_ma		"Aspired education for son at least MA (yes=1)"
...
label var asp_educ_kids_high		"Avg educ aspirations for children above md (yes=1)"
...
label var shop_house_sep "Business premises separate from residence (yes/no)"
...
foreach var in	sales_lastmth sales_nday prof_lastmth prof_nday prof_est prof_est2 {
				
				winsor2 `var', cuts(1 99) 		suffix(_w1)
				winsor2 `var', cuts(2.5 97.5)	suffix(_w25)
				winsor2 `var', cuts(5 95) 		suffix(_w5)
				gen `var'_ihs = ln(`var' + sqrt((`var'*`var') + 1))
				local lbl : var label `var'
				label var `var'_w1  "`lbl' (win 1%)"
				label var `var'_w25  "`lbl' (win 2.5%)"
				label var `var'_w5  "`lbl' (win 5%)"
				label var `var'_ihs  "`lbl' (IHS)"
