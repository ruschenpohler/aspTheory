...
* Decimal degrees
gen gps_dec_lat = -1 * (gps_dms_lat_deg + (gps_dms_lat_min/60))
...
* Recode label
gen top`x'_prod = substr(f4_5_1_cd`x',1,2)

* Calculate fraction of sales from prod `x' of total daily sales
gen top`x'sales_frac = prod`x'/sales_nday

* Type of prod `x'
gen prod`x'_tuksayur		= 1 if 	inlist(top`x'_prod,"01","02","03","04","05","06","07")
replace prod`x'_tuksayur	= 0 if prod`x'_tuksayur==.

gen prod`x'_rokok			= 1 if 	top`x'_prod=="24"
replace prod`x'_rokok 		= 0 if prod`x'_rokok==.

gen prod`x'_gas				= 1 if 	top`x'_prod=="29"
replace prod`x'_gas 		= 0 if prod`x'_gas==.

gen prod`x'_warung			= 1 if 	inlist(top`x'_prod,"08","09","10","11","12","13","14","15","16") ///
									| inlist(top`x'_prod,"17","18","19","20","21","22","23","25","26") ///
									| inlist(top`x'_prod,"27","28","30","31")
replace prod`x'_warung 		= 0 if prod`x'_warung==.

* Calculate total fraction of each type for all three top prods of total daily sales
replace tuksayur 	= tuksayur 	+ (prod`x'_tuksayur * top`x'sales_frac)
replace rokok 		= rokok 	+ (prod`x'_rokok 	* top`x'sales_frac)
replace gas 		= gas 		+ (prod`x'_gas 		* top`x'sales_frac)
replace warung 		= warung 	+ (prod`x'_warung 	* top`x'sales_frac)

replace tuksayur2 	= tuksayur2 + (prod`x'_tuksayur * prod`x')
replace rokok2		= rokok2 	+ (prod`x'_rokok 	* prod`x')
replace gas2 		= gas2 		+ (prod`x'_gas 		* prod`x')
replace warung2 	= warung2 	+ (prod`x'_warung 	* prod`x')

}


* Determine shop type according to category that contributes most to sales (1=veggies/fruits/meat/fish, 2=cigarettes, 3=cooking gas, 4=misc mom and pop)
gen shoptype = 0
replace shoptype = 1 if tuksayur>rokok & tuksayur>gas & tuksayur>warung
replace shoptype = 2 if rokok>=tuksayur & rokok>gas & rokok>warung
replace shoptype = 3 if gas>=tuksayur & gas>=rokok & gas>warung
replace shoptype = 4 if warung>=tuksayur & warung>=rokok & warung>=gas
tab shoptype, gen(shoptype)

* Alternatively: Classify into either of the categories if shop makes more than 60k rupiah (==4 packs of cigarettes, 6kg of LPG) in sales from that category
foreach x in 	tuksayur2 rokok2 gas2 warung2 {

				replace `x' = 0 if `x'<=14.80
				replace `x' = 1 if `x'>14.80 & !missing(`x')
...
* Topcoding
//replace asp_shop_z = 3.754558 if asp_shop_z>3.75
...
* Topcoding
//replace asp12_shop_z = 3.635172 if asp12_shop_z>3.635172
...
foreach x of varlist asp_import asp_prob asp_seff asp_loc asp_cse {
...
label var asp12_sales_seff "12mth sales aspirations weighted by perceived self-efficacy"
...
label var asp12_shop_z_seff "12mth shop aspirations weighted by perceived self-efficacy"

label var asp12_shop_z_loc "12mth shop aspirations weighted by perceived locus of control"

label var asp12_shop_z_cse "12mth shop aspirations weighted by perceived agency"
...
replace asp_occup_dtr_oth = 6 if f4_10_5c_ot=="RETAILERS"
replace asp_occup_dtr_oth = 7 if f4_10_5c_ot=="MINING" | f4_10_5c_ot=="BLACKSMITH"
replace asp_occup_dtr_oth = 8 if f4_10_5c_ot=="MACHINIST"
replace asp_occup_dtr_oth = 11 if f4_10_5c_ot=="MINI MART SUPERVISOR"
replace asp_occup_dtr_oth = 13 if f4_10_5c_ot=="ADMINISTRATION"
replace asp_occup_dtr_oth = 14 if inlist(f4_10_5c_ot,"HOTEL","OFFICE MANAGER","MANAGER","OFFICE SUPERVISOR","CHEF") 
replace asp_occup_dtr_oth = 19 if f4_10_5c_ot=="ARMY" | f4_10_5c_ot=="NAVY"
replace asp_occup_dtr_oth = 20 if inlist(f4_10_5c_ot,"CS IN PROVINCE AND MINISTERIAL LEVEL","GOVERNOR")
replace asp_occup_dtr_oth = 25 if inlist(f4_10_5c_ot,"ENGINEER","ELECTRONIC ENGINEER","OIL AND GAS ENGINEER","MECHANICAL ENGINEER")
replace asp_occup_dtr_oth = 27 if f4_10_5c_ot=="BANKER" | f4_10_5c_ot=="BANK MANAGER"
replace asp_occup_dtr_oth = 28 if f4_10_5c_ot=="MINISTER"

* Social status not clear, but certainly non-govt
gen asp_occup_dtr_nongovt = 1	if 	inlist(f4_10_5c_ot,"ENTREPRENEUR","CLINIC ENTREPRENEUR","IT ENTREPRENEUR","MEDIA ENTREPRENEUR","IT","PROGRAMMER","SAILORMAN","PREACHER","WORKSHOP") | ///
...
gen aspgap_educ_son = (asp_educ_son - educ)/educ
replace aspgap_educ_son = 5 if aspgap_educ_son>5 & aspgap_educ_son!=.

* Aspirations for oldest daughter U18
gen asp_educ_dtr = f4_10_5b
gen aspgap_educ_dtr = (asp_educ_dtr - educ)/educ
replace aspgap_educ_dtr = 5 if aspgap_educ_dtr>5 & aspgap_educ_dtr!=.
...
gen aspgap_educ_kids = (asp_educ_kids - educ)/educ
replace aspgap_educ_kids = 5 if aspgap_educ_kids>5 & aspgap_educ_kids!=.


* Dummies

foreach x of varlist	asp_educ_son asp_educ_dtr asp_educ_kids {
...
foreach x of varlist	asp_educ_son asp_educ_dtr asp_educ_kids ///
						aspgap_educ_son aspgap_educ_dtr aspgap_educ_kids {
...
//savesome shop_ID shoptype* tuksayur rokok gas warung using "Data\Baseline Data\W1_shoptype.dta", replace
