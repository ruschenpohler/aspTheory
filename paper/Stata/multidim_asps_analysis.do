...
* Business Performance and Size (Add Variables Here!)
local perform	prof_est prof_est_w1 prof_est_w2 ///
				sales_lastmth sales_lastmth_w1 sales_lastmth_w2 ///
				
local size		labour_total labour_nonfam labour_fam ///
				size

* Aspirations (Add Variables Here!)
...
foreach wave in `Waves' {
	...
	* Confirming endline data exists
	...
	capture erase "csv\multidim asps\treatment effects\T`m'_`wave'_AllOutcomes_itt.csv"

	* Excel output
	#delimit ;
		esttab T`m'_`wave'_AllOutcomes_* using "csv\multidim asps\treatment effects\T`m'_`wave'_AllOutcomes_itt.csv", 
...
foreach wave in `Waves' {
	...
	* Confirming endline data exists
	...
	capture erase "csv\multidim asps\treatment effects\T`m'_`wave'_AllOutcomes_tot.csv"

	* Excel output
	#delimit ;
		esttab T`m'_`wave'_AllOutcomes_* using "csv\multidim asps\treatment effects\T`m'_`wave'_AllOutcomes_tot.csv", 
		label replace modelwidth(16) varwidth(50) depvar legend 
		cells(b(star fmt(%9.3f)) se(par)) star(* 0.10 ** 0.05 *** 0.01)
...
foreach wave in `Waves' {
				...
				if !_rc {
					...
					* Storing Control Group Stats by Levels of Heterogeneous Variable
					...
					if regexm("`y'", "asp_") == 1 {
						...
						* Estimating OLS Specification With Baseline Dep Var and Strata Controls
						display "HTE: With BL Dep Var and Strata Controls"
						areg `wave'_`y' W1_asp_yrs `treat' `hte_`y'' W1b_`y' W1_`y'_m `strata' if `wave'_finished==1, absorb(W1_village) robust
					...
					* Making graphs
					margins `treat'#`hte_asp12_shop_z'
					marginsplot, recast(scatter)
					margins, dydx(Post) at(`treat' = (0 1))
					graph save "csv\multidim asps\treatment effects\fig`m'_`i'", replace
	...
	capture erase "csv\multidim asps\treatment effects\T`m'_`wave'_BisAsps_hte.csv"
	
	* output
	#delimit ;
		esttab T`m'_`wave'_BisAsps_hte* using "csv\multidim asps\treatment effects\T`m'_`wave'_BisAsps_hte.csv", label replace modelwidth(16) varwidth(50) depvar legend 
		cells(b(star fmt(%9.3f)) se(par)) star(* 0.10 ** 0.05 *** 0.01) 
		stats(r2 N 
		...
		title(Wave `wave': Heterogeneity in Impact on Business Aspirations by Outcome Level at Baseline) keep(W1_asp_yrs B*);
	#delimit cr
...
foreach wave in `Waves' {
	...
	foreach y in `perform' `size' `satisfact' {
	...
	* Confirming endline data exists
				...
				if !_rc {
					...
					* Generating Dummies for missing BL vars 
					...
					foreach hte_var in `hte' {
						...
						* Generating local for HTE dummies
						local hte_`hte_var' B_`hte_var'_BM BM_`hte_var'_BM BC_`hte_var'_BM BMC_`hte_var'_BM W1_`hte_var'
						...
						* Storing Estimates
						eststo T`m'_`wave'_PerfSizeSat_hte`i'
						estadd ysumm
	...
	capture erase "csv\multidim asps\treatment effects\T`m'_`wave'_PerfSizeSat_hte.csv"
	
	* output
	#delimit ;
		esttab T`m'_`wave'_PerfSizeSat_hte* using "csv\multidim asps\treatment effects\T`m'_`wave'_PerfSizeSat_hte.csv", label replace modelwidth(16) varwidth(50) depvar legend 
		cells(b(star fmt(%9.3f)) se(par)) star(* 0.10 ** 0.05 *** 0.01) 
		stats(r2 N 
		mean_asp_shop_z_AM sd_asp_shop_z_AM mean_asp_shop_z_BM sd_asp_shop_z_BM 
		mean_asp12_shop_z_AM sd_asp12_shop_z_AM mean_asp12_shop_z_BM sd_asp12_shop_z_BM 
...
local ses			W1_male W1_age_manager W1_kids_3 W1_educ
local firm			W1_age_firm W1_formal_tax W1_size W1_prof_est_w1 W1_labour_total /*W1_sales_lastmth*/
local pract			W1_MW_M_score_total W1_MW_B_score_total W1_MW_R_score_total W1_MW_F_score_total W1_prods_new_5
local psych			W1_digitspan_total W1_time_comp W1_risk_comp
...
capture erase "csv\multidim asps\treatment effects\T`m'_AMchars.csv"
	
* output
#delimit ;
	esttab T`m'_AMchars* using "csv\multidim asps\treatment effects\T`m'_AMchars.csv", label replace modelwidth(16) varwidth(50) depvar legend 
	cells(b(star fmt(%9.3f)) se(par)) star(* 0.10 ** 0.05 *** 0.01) 
...
foreach y in		asp12_shop_z_AM asp_shop_z_AM {
	...
	foreach x in 	`firm' `perform' `entrep' {
		...
		* Generating Dummies for missing BL vars 
		foreach var in ``x'' {
			...
			gen W1_`var'_m = (`var'==.)
			gen W1_`var'_b = `var'
			replace W1_`var'_b = 5 if `var'_m == 1
						
			local lbl: var label W1_`var'
			label var W1_`var'_b "`lbl'"
			
			local x_vect `x_vect' W1_`var'_m W1_`var'_b	
		}
...
foreach wave in `Waves' {
	...
	* Confirming endline data exists
	...
	capture erase "csv\multidim asps\treatment effects\T`m'_`wave'_Asps_hte_byGenderSize.csv"
	
	* output
	#delimit ;
		esttab T`m'_`wave'_Asps_hte_byGenSize* using "csv\multidim asps\treatment effects\T`m'_`wave'_Asps_hte_byGenderSize.csv", label replace modelwidth(16) varwidth(50) depvar legend 
		cells(b(star fmt(%9.3f)) se(par)) star(* 0.10 ** 0.05 *** 0.01) 
